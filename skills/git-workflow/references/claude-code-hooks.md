...
The write is found by scanning tokens rather than by one pattern, because a pattern that knew only `-c` missed `git --no-pager push` and one that required a statement separator missed `env X=1 git push`. Global options are skipped until the subcommand, `-C` and `--git-dir` among them count as naming the directory, and a `cd` only counts with a real operand — `cd && git push` and `cd - && git push` leave the inherited directory in place. Quoted runs are blanked before the scan, so `git -C /r commit -m "fix git push"` is not read as a second, unnamed write; the accepted cost is that a payload a wrapper would run (`bash -c 'git push'`) is not seen either, and a test pins that gap. The escape hatch counts only where a shell would read it as an environment assignment, at the start of a command: as a substring it also disabled the gate from inside a commit message that merely named it, which the messages in this repository do. That predicate is shared with `blanket_git_add()`, which had the same weakness.
